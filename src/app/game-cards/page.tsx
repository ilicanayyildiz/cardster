...
export default function GameCardsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await fetch(`/api/products?category=gamecards`, { next: { revalidate: 60 } });
  ...
  useEffect(() => {
    ...
    if (qpCountry && !segCountry) {
      ...
      router.replace(`/${qpCountry}/game-cards${suffix ? `?${suffix}` : ""}`);
    }
  ...
  return (
    <div className="bg-white min-h-[70vh]">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-semibold text-gray-900">Game Cards for Every Gamer</h1>

        <div className="mt-4 flex flex-wrap items-center gap-6 text-sm">
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Instant Codes</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Official or Authorized Sellers</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Secure Payments</span></span>
        </div>
                  ...
                  {filteredCountries.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        sp.delete("country");
                        const suffix = sp.toString();
                        router.push(`/${c.code}/game-cards${suffix ? `?${suffix}` : ""}`);
                        router.refresh();
                      }}
        ...
        <div className="mt-8 grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {loading ? (
            ...
            products.map((p) => (
              <a
                key={p.slug}
                href={`/${country}/game-cards/${encodeURIComponent(p.slug)}`}
                className="block rounded-xl border border-gray-200 bg-white hover:shadow transition-shadow"
              >
                <div className="relative w-full h-28 sm:h-32">
                  <Image src={`${imageBase}${p.slug}.png${versionQuery}`} alt={p.name} fill sizes="(max-width:768px) 100vw, 240px" className="object-contain p-4" />
                </div>
              </a>
            ))
          )}
