...
export default function EntertainmentGiftCardsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await fetch(`/api/products?category=entertainment`, { next: { revalidate: 60 } });
  ...
  useEffect(() => {
    ...
    if (qpCountry && !segCountry) {
      ...
      router.replace(`/${qpCountry}/entertainment-gift-cards${suffix ? `?${suffix}` : ""}`);
    }
  ...
  return (
    <div className="bg-white min-h-[70vh]">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-semibold text-gray-900">Entertainment Gift card, Delivered Instantly</h1>

        <div className="mt-4 flex flex-wrap items-center gap-6 text-sm">
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Enjoy Credits Instantly</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Authorized Provider</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">100% Protected Transactions</span></span>
        </div>
                  ...
                  {filteredCountries.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        sp.delete("country");
                        const suffix = sp.toString();
                        router.push(`/${c.code}/entertainment-gift-cards${suffix ? `?${suffix}` : ""}`);
                        router.refresh();
                      }}
        ...
        <div className="mt-8 grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {loading ? (
            ...
            products.map((p) => (
              <a
                key={p.slug}
                href={`/${country}/entertainment-gift-cards/${encodeURIComponent(p.slug)}`}
        ...
        <section className="mt-10">
          <h2 className="text-xl font-semibold text-gray-900">Purpose of Entertainment Gift Cards</h2>
          <p className="mt-3 text-gray-700 leading-relaxed">
            Entertainment gift cards are fabulous gifts for family and friends. Cardster entertainment cards work instantly.
            Browse through a wide range of entertainment gift cards and you will surely find one for your beloved friend or family member.
          </p>
        </section>

        <section className="mt-8">
          <h2 className="text-xl font-semibold text-gray-900">Entertainment Gift Cards for Yourself</h2>
          <p className="mt-3 text-gray-700 leading-relaxed">
            These amazing cards are perfect for pampering yourself, too. They are cost‑effective alternatives to full‑term subscriptions and can recharge your services anytime, anywhere.
          </p>
        </section>

        <section className="mt-8">
          <h2 className="text-xl font-semibold text-gray-900">How Can I Get My Entertainment Cards?</h2>
          <ol className="mt-3 list-decimal pl-6 space-y-2 text-gray-700">
            <li>Visit Cardster.</li>
            <li>Choose the Entertainment Card and value.</li>
            <li>Pay securely (cards and wallets supported).</li>
            <li>Receive your gift code within seconds by email.</li>
          </ol>
        </section>
      </div>
    </div>
