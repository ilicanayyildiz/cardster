...
function TravelInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await fetch(`/api/products?category=travel`, { next: { revalidate: 60 } });
  ...
  useEffect(() => {
    ...
    if (qpCountry && !segCountry) {
      ...
      router.replace(`/${qpCountry}/travel${suffix ? `?${suffix}` : ""}`);
    }
  ...
  return (
    <div className="bg-white min-h-[70vh]">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-semibold text-gray-900">Travel Gift Cards For Travelers and Adventure‑Seekers</h1>

        <div className="mt-4 flex flex-wrap items-center gap-6 text-sm">
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Digital Access Immediately</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Certified Seller</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Fraud‑proof Transactions</span></span>
        </div>
                  ...
                  {filteredCountries.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        sp.delete("country");
                        const suffix = sp.toString();
                        router.push(`/${c.code}/travel${suffix ? `?${suffix}` : ""}`);
                        router.refresh();
                      }}
        ...
        <div className="mt-8 grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {loading ? (
            ...
            products.map((p) => (
              <a
                key={p.slug}
                href={`/${country}/travel/${encodeURIComponent(p.slug)}`}
        ...
        <section className="mt-10">
          <h2 className="text-xl font-semibold text-gray-900">Travel Cards - The Perfect Gift</h2>
          <p className="mt-3 text-gray-700 leading-relaxed">
            Travel gift cards are a great choice for anyone who loves to explore the world. Booking flights, cruises,
            local travel, hotels, and other services is much simpler with travel cards.
          </p>
        </section>

        <section className="mt-8">
          <h2 className="text-xl font-semibold text-gray-900">Where to Buy Travel Cards Online?</h2>
          <p className="mt-3 text-gray-700 leading-relaxed">
            Choose from a range of options from flights to hotels. It’s quick and easy to order.
          </p>
        </section>
      </div>
    </div>
