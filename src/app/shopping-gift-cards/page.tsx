...
function ShoppingInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await fetch(`/api/products?category=shopping`, { next: { revalidate: 60 } });
  ...
  useEffect(() => {
    ...
    if (qpCountry && !segCountry) {
      ...
      router.replace(`/${qpCountry}/shopping-gift-cards${suffix ? `?${suffix}` : ""}`);
    }
  ...
  return (
    <div className="bg-white min-h-[70vh]">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-semibold text-gray-900">Shopping Gift Cards for Last‑Minute Requirements</h1>

        <div className="mt-4 flex flex-wrap items-center gap-6 text-sm">
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Fast Delivery</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Top reseller</span></span>
          <span className="inline-flex items-center gap-2"><span className="text-yellow-500">✔</span> <span className="text-gray-900">Fully‑secured Transactions</span></span>
        </div>
                  ...
                  {filteredCountries.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        sp.delete("country");
                        const suffix = sp.toString();
                        router.push(`/${c.code}/shopping-gift-cards${suffix ? `?${suffix}` : ""}`);
                        router.refresh();
                      }}
        ...
        <div className="mt-8 grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {loading ? (
            ...
            products.map((p) => (
              <a
                key={p.slug}
                href={`/${country}/shopping-gift-cards/${encodeURIComponent(p.slug)}`}
        ...
        <section className="mt-10">
          <h2 className="text-xl font-semibold text-gray-900">Shopping Cards: Perfect for Gifting Anytime, Anywhere</h2>
          <p className="mt-3 text-gray-700 leading-relaxed">
            Need to order a gift urgently? Choose from a range of shopping cards and gifts to meet individual preferences.
          </p>
        </section>

        <section className="mt-8">
          <h2 className="text-xl font-semibold text-gray-900">Getting Your Shopping Card</h2>
          <ol className="mt-3 list-decimal pl-6 space-y-2 text-gray-700">
            <li>Select the preferred card and amount.</li>
            <li>Pay securely (PayPal, cards and more).</li>
            <li>Receive the card instantly in your inbox.</li>
          </ol>
        </section>
      </div>
    </div>
